...
struct DashboardView: View {
    ...
    private func optimizeMemory() {
        ...
        // `vm_kernel_page_size` 는 전역 가변 변수라 동시성 안전하지 않다.
    ...
    private func updateSystemStats() {
        ...
        if cpuName.isEmpty {
            ...
            if size > 0 {
                ...
                // 널 종료 문자를 잘라낸 뒤 디코딩한다. (String(cString:) 는 deprecated)
                let bytes = brand.prefix { $0 != 0 }
                cpuName = String(decoding: bytes, as: UTF8.self).trimmingCharacters(in: .whitespacesAndNewlines)
            } else {
