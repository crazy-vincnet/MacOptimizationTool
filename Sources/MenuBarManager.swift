...
final class MenuBarManager: NSObject, ObservableObject {
    ...
    @Published var cpuUsage: Double = 0.05
    @Published var memoryUsage: Double = 0.35
    ...
    private func updateStats() {
        let ramStats = HardwareStatsHelper.getRAMStats()
        if ramStats.total > 0 {
        ...
        if let button = statusItem?.button {
            let memInt = Int(self.memoryUsage * 100)
            button.title = " \(memInt)%"
        }
    }
        ...
        VStack(spacing: 16) {
            ...
            HStack(spacing: 12) {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text("\(Int(menuManager.memoryUsage * 100))%")
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("CPU 사용량")
                        .font(.system(size: 11))
                        .foregroundColor(Theme.textSecondary)
                    Text("\(Int(menuManager.cpuUsage * 100))%")
                        .font(.system(size: 20, weight: .bold))
                        .foregroundColor(Theme.accent)
                }
                .frame(maxWidth: .infinity, alignment: .leading)
            ...
            VStack(spacing: 8) {
                ...
                HStack {
                    Label("업로드", systemImage: "arrow.up.circle.fill")
                        .font(.system(size: 12))
                        .foregroundColor(Theme.textSecondary)
                    Spacer()
