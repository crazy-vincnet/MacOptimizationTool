...
struct StartupManagerView: View {
    ...
    private var headerView: some View {
        HStack(alignment: .center) {
            VStack(alignment: .leading, spacing: 4) {
                Text("시동 프로그램 & 백그라운드 관리")
                    .font(.system(size: 22, weight: .bold))
                    .foregroundColor(Theme.textPrimary)
                Text("Mac 부팅 시 백그라운드에서 실행되어 자원을 점유하는 에이전트를 관리합니다.")
                    .font(.system(size: 13))
                    .foregroundColor(Theme.textSecondary)
            }
    ...
    private var filterPills: some View {
        HStack(spacing: 10) {
            filterButton(title: "전체 (\(viewModel.startupItems.count))", type: nil)

            ForEach(StartupType.allCases, id: \.self) { type in
                let count = viewModel.startupItems.filter { $0.type == type }.count
                filterButton(title: "\(type.rawValue) (\(count))", type: type)
            }
