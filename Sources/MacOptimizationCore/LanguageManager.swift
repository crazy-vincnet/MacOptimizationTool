...
/// 번역 사전은 불변이지만 선택된 언어는 런타임에 바뀌고, 번역은 백그라운드 태스크에서도 읽는다.
/// 따라서 현재 언어 스냅샷만 락으로 보호하고 조회 자체는 액터 격리 없이 수행한다.
public final class LanguageManager: ObservableObject, @unchecked Sendable {
    public static let shared = LanguageManager()

    private static let languageLock = NSLock()
    nonisolated(unsafe) private static var cachedEffectiveLanguage: AppLanguage = .english

    @Published public var currentLanguage: AppLanguage = .system {
        didSet {
            UserDefaults.standard.set(currentLanguage.rawValue, forKey: "appLanguage")
            Self.updateCachedLanguage(from: currentLanguage)
    ...
    private init() {
        ...
        Self.updateCachedLanguage(from: self.currentLanguage)
    }

    private static func updateCachedLanguage(from selected: AppLanguage) {
        let resolved = selected == .system ? AppLanguage.detectSystemLanguage() : selected
        languageLock.lock()
        cachedEffectiveLanguage = resolved
        languageLock.unlock()
    }

    /// 스레드 안전한 언어 스냅샷.
    public nonisolated static var snapshotLanguage: AppLanguage {
        languageLock.lock()
        defer { languageLock.unlock() }
        return cachedEffectiveLanguage
    }

    /// 어떤 스레드에서도 안전하게 호출할 수 있는 번역 조회.
    public nonisolated static func translate(_ key: String) -> String {
        // 싱글턴이 아직 만들어지지 않았다면 여기서 초기화해 저장된 언어 설정을 반영한다.
        _ = shared
        let lang = snapshotLanguage
    ...
    /// 기본 사전 + 뷰별 생성 사전(GeneratedTranslations) + 추가 사전(AdditionalTranslations) 병합 결과.
    private static let translations: [AppLanguage: [String: String]] = {
        var merged = baseTranslations
        for source in [GeneratedTranslations.all, AdditionalTranslations.all] {
            for (lang, dict) in source {
...
/// 전역 번역 단축 헬퍼 함수. 백그라운드 스레드에서도 호출되므로 락으로 보호된 스냅샷을 읽는다.
public func t(_ key: String) -> String {
